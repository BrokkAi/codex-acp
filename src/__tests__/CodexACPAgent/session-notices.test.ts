...
describe("session notices", () => {
    ...
    it("buffers child notices until the child session exists and stops them when it closes", async () => {
        ...
        }, modelRerouted(childSessionId, "child-turn")]);
...
function modelRerouted(threadId: string, reroutedTurnId = turnId): ServerNotification {
    return {
        method: "model/rerouted",
        params: {threadId, turnId: reroutedTurnId, fromModel: "original-model", toModel: "fallback-model", reason: "highRiskCyberActivity"},
    };
}
